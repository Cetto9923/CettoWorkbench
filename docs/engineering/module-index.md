...
> [!NOTE]
> Listing here documents current source reality; it does **not** certify any legacy module as a golden architectural reference. Table ownership is marked `SCHEMA UNVERIFIED` until production schema DDL is confirmed via Phase V0.

| Module | Responsibility | Public Services | Dependencies | Table Ownership | Status | Source Location |
|---|---|---|---|---|---|---|
| `dept` | Department tree structure and CRUD | `dept.Service` | `dept.Repo`, `gorm.DB` | `zt_dept` | SOURCE VERIFIED / SCHEMA UNVERIFIED | `internal/module/dept/` |
...
| `role` | Role definitions and capability assignments | `role.Service` | `role.Repo`, `gorm.DB` | `zt_role`, `zt_role_priv` | SOURCE VERIFIED / SCHEMA UNVERIFIED | `internal/module/role/` |
| `user` | User management (legacy non-authoritative) | `user.Service` | `user.Repo`, `gorm.DB` | `zt_user`, `zt_user_role` | SOURCE VERIFIED / SCHEMA UNVERIFIED (Legacy) | `internal/module/user/` |
