...
(function () {
  ...
  var objectTypeBadge = PL.objectTypeBadge || function (kind) {
    var labels = { business: "业务需求", story: "研发需求", project: "项目", task: "任务", bug: "Bug", testtask: "测试单", issue: "问题", risk: "风险", approval: "审批", mail: "邮件" };
  ...
  function formatNoticeSubject(item) {
    ...
    var cleanTitle = rawSubject;
    var badgeHtml = "";
    if (ot && ot !== "mail" && OBJECT_TYPE_LABELS[ot]) {
      var badgeText = OBJECT_TYPE_LABELS[ot] + (oid && oid !== "0" ? " #" + oid : "");
      badgeHtml = '<span class="notice-tag notice-tag-' + esc(ot) + '">' + esc(badgeText) + "</span>";
      cleanTitle = cleanTitle.replace(/^(?:[a-zA-Z\u4e00-\u9fa5]+)\s*#?\d+\s*[-:：· ]*/i, "").trim();
      if (!cleanTitle) cleanTitle = rawSubject;
    }

    var subText = "";
    var rawSummary = String(item.data || item.summary || "").trim();
    if (rawSummary && rawSummary !== rawSubject && rawSummary !== cleanTitle) {
      var icon = "";
    ...
    return '<div class="notice-subject-text" title="' + esc(rawSubject) + '">' +
      badgeHtml + '<button type="button" class="notice-title-main" data-notice-open="' + esc(item.id) + '">' + esc(cleanTitle) + "</button></div>" + subText;
  }
  ...
  function openNoticeDrawer(id) {
    ...
    var ot = String(item.objectType || "").trim().toLowerCase();
    var oid = String(item.objectId || "").trim();
    var objName = (OBJECT_TYPE_LABELS[ot] || ot || "—") + (oid && oid !== "0" ? " #" + oid : "");
