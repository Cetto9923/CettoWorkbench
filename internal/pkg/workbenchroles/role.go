// =============================================================================
// 文件: internal/pkg/workbenchroles/role.go
// 模块: 基础设施
// 类型: infra
// 职责: 定义工作台角色 key、角色字典与默认允许角色推导（纯静态规则，不依赖 DB）。
// 依赖: 无
// =============================================================================

// Package workbenchroles 提供工作台角色 key、角色字典以及默认允许角色的
// 静态推导规则。规则不依赖数据库或会话，便于在请求早期（路由中间件、模板
// 渲染、权限预筛）被复用。
//
// 本包是工作台角色语义的最小骨架；后续可在此基础上扩展按部门、能力、租户
// 等维度的细分规则，但 MUST 保持纯静态推导、不引入数据库依赖。
