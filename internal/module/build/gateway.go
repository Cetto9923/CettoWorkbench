...
type linkBuildStoriesReq struct {
	BuildID uint
	Account string // 当前登录账号；走 DoAs
	Stories string // 逗号分隔需求 ID
}

// linkBuildStories 调用禅道关联研发需求接口（用户态 DoAs）。
...
// unlinkBuildStories 调用禅道解除版本与研发需求关联接口（用户态 DoAs）。
...
func postBuildStories(ctx context.Context, client *zentao.Client, req linkBuildStoriesReq, action string) error {
	...
	return client.DoAs(ctx, account, http.MethodPost, path, payload, nil)
}
