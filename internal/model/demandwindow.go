...
type DemandWindow struct {
	...
	DemandID   uint           `gorm:"column:demand;not null;index:idx_demand;uniqueIndex:uk_demand_story_versionWindow_product" json:"demandId"`
	StoryID    uint           `gorm:"column:story;not null;default:0;index:idx_story;uniqueIndex:uk_demand_story_versionWindow_product" json:"storyId"`
	WindowID   uint64         `gorm:"column:versionWindow;not null;index:idx_versionWindow;uniqueIndex:uk_demand_story_versionWindow_product" json:"windowId"`
	PlanID     *uint          `gorm:"column:plan" json:"planId"`
	ProductID  uint           `gorm:"column:product;not null;index:idx_product;uniqueIndex:uk_demand_story_versionWindow_product" json:"productId"`
